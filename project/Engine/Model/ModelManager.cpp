...
void ModelManager::Update(SkinCluster& skinCluster, const Skeleton& skeleton) {
	for (size_t jointIndex = 0; jointIndex < skeleton.joints.size(); ++jointIndex) {
		...
		skinCluster.mappedPalette[jointIndex].skeletonSpaceInverseTransposeMatrix = Matrix::Transpose(Matrix::Inverse(skinCluster.mappedPalette[jointIndex].skeletonSpaceMatrix));
	}
}
