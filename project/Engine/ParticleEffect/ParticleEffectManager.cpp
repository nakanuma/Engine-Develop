...
void ParticleEffectManager::Register(const std::string& name, const std::shared_ptr<IParticleEffect>& effect) { 
	// 引数の名前をキーにしてマップへ格納
	effects_[name] = std::move(effect); 
}

void ParticleEffectManager::Emit(const std::string& name, const Float3& pos, uint32_t count, const Float3& velocity, float angle) {
	// 登録時の名前があれば発生処理
